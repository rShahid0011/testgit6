...
#define TEMPERATURE_C_LIMIT_WARNING 32.0
...
bool gasDetectorStateRead() { return gasDetectorState; }

bool overTemperatureDetectorStateRead() { return overTemperatureDetectorState; }

bool gasDetectedRead() { return gasDetected; }

bool overTemperatureDetectedRead() { return overTemperatureDetected; }
...
static void fireAlarmActivationUpdate() {
  ...
  overTemperatureDetectorState =
      temperatureSensorReadCelsius() > TEMPERATURE_C_LIMIT_ALARM;
  ...
  if (coldStart)
  {
      //enter code to deactivate alarm after reset
    overTemperatureDetected = ON;
    gasDetected = ON;
    sirenStateWrite(ON);
    strobeLightStateWrite(ON);
